...
export default function PulseApp() {
  ...
  const handleReorder = (newOrder: Task[]) => {
    // Only reordering active tasks in this view, but we need to merge back into main list
    // This is a bit tricky with filtering. 
    // For simplicity in this prototype, we'll just update the state with the reordered list 
    // assuming we are only reordering the currently visible list and appending the rest.
    // However, a better approach for a real app is to have a 'order' field.
    // Here we will just replace the tasks that match the current filter with the new order.
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-100">
      ...
      {/* 1. Responsive Constraint Overlay */}
      <div className="hidden sm:flex fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xl items-center justify-center p-8 text-center">
        <div className="bg-white p-8 rounded-2xl shadow-2xl max-w-md flex flex-col items-center space-y-4">
          <Smartphone className="w-16 h-16 text-slate-400" />
          <h2 className="text-2xl font-bold text-slate-900">Mobile View Only</h2>
          <p className="text-slate-600">
            Please resize your browser window to a smaller width (mobile size) to interact with the Pulse prototype.
          </p>
        </div>
      </div>

      {/* Main App Content - Hidden on Desktop */}
      <div className="sm:hidden flex flex-col min-h-screen relative overflow-hidden">
        ...
        <div className="sticky top-[61px] z-20 bg-slate-50 pt-2 pb-2 px-4 border-b border-slate-200 flex space-x-2 overflow-x-auto no-scrollbar">
        ...
        {/* 2. Timeline View */}
        <main className="flex-1 px-4 py-4 relative">
          {/* Timeline Line */}
          ...
          {/* Tooltip Hint */}
          {activeTab === 'active' && filteredTasks.length > 0 && !isReorderMode && (
             <motion.div 
               initial={{ opacity: 0, y: 10 }}
               animate={{ opacity: 1, y: 0 }}
               transition={{ delay: 1, duration: 0.5 }}
               className="fixed bottom-24 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-xs px-3 py-1.5 rounded-full shadow-lg pointer-events-none z-10"
             >
        ...
        <div className="fixed bottom-6 right-6 flex flex-col items-center space-y-4 z-40">
          <motion.button 
            whileTap={{ scale: 0.9 }}
          ...
          {isProfileOpen && (
            <>
              ...
              <motion.div 
                ...
                className="fixed top-0 left-0 bottom-0 w-4/5 max-w-xs bg-white z-50 shadow-2xl overflow-y-auto"
              >
